...
const getEmail = tool(
  () => {
    // TODO : call gmail apis to get emails from gmail
    return JSON.stringify(gmailEmails);
  },
  {
    name: "get_email",
    description: "Get emails information from gmail",
  }
);
...
const refund = tool(
  ...
  },
  {
    ...
    schema: z.object({
      emails: z
        .array(z.string())
        .describe("The list of emails which needed to be refunded"),
    }),
  }
);
    ...
    while(true){
        ...
        const response=await agent.invoke(
          ...
          if(response?.__interrupt__?.length){
              ...
              output+="Choose : " + "\n"
              output+=response.__interrupt__[0].value.reviewConfigs[0].allowedDecisions.filter((dec)=>dec!=="edit").map((dec,i)=> `${i+1}. ${dec}`).join("\n")
          }else{
