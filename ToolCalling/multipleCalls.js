    ...
    while (true) {
        ...
        for (const toolCall of toolCalls) {
          ...
          if (functionName === "getHotels") {
            const toolResult = await getHotels(JSON.parse(functionParam));
            //  console.log("toolResult:",toolResult)
            messages.push({
              tool_call_id: toolCall.id,
              role: "tool",
...
async function getHotels (args)  {
    return JSON.stringify({
      city: args.city,
      hotels: [
        { name: "Grand Hotel", price: 120, rating: 4.5, available: true },
        { name: "City Inn", price: 85, rating: 4.2, available: true },
        { name: "Luxury Resort", price: 250, rating: 4.8, available: false }
      ]
    });
  }
